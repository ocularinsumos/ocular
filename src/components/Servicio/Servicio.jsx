...
export default function Servicio({ titulo, texto, imagen, texto2, texto3, icono, prod }) {
  ...
  return (
    <section className="min-h-screen grid col-span-1 justify-items-center md:grid-cols-3 md:pl-20 md:pr-10 py-16 gap-10 md:gap-20 ">
      ...
      <article className="relative w-full col-span-2 p-2"> 
    ...
    {prod && prod.map((item, index) => (
      <li key={index} className="flex flex-row items-center bg-primary border hover:bg-primary-hover border-gray-200 rounded-lg shadow max-h-full mx-auto max-w-4xl">
        ...
        <div className="flex flex-col justify-between p-4 leading-normal w-full ">
          <h2 className="mb-2 text-base md:text-2xl font-bold tracking-tight text-gray-300">{item.nombre}</h2>
          <p className="mb-2 text-sm md:text-base font-normal text-gray-300">{item.descripcion}</p>
          <p className="mb-2 text-sm md:text-base font-normal text-gray-300">{item.marca}</p>
